...
- [x] API surfaces cho create / parse / search / simulate / report đã có.
...
- [x] Local SQLite persistence và attachment upload flow đã có cho Phase 2 backend.
- [x] Phase 2 backend/API đã hoàn tất; hạng mục evidence table UI được dời sang phase frontend.
- [x] Phase 3 simulation runtime đã chạy local CPU với flow retrieval -> plaintiff -> defense -> judge -> clerk.
- [x] Phase 4 harness baseline đã chạy local CPU với audit trail và human review gate.
- [x] Phase 5 backend baseline đã có human review resolution, `report_ready`, và markdown export.
- [x] Provider abstraction cho OpenRouter, Groq, 9Router, và Ollama Cloud đã có với heuristic fallback cho runtime tranh tụng.
- [x] Backend MVP scope đã hoàn tất ở mức scripted demo.
- [x] Frontend workspace thật đã có trong `frontend/`; lane UI MVP/V1/V2 đã được nối với backend.
...
- [x] Phase 2 completed for backend/API scope
...
- [x] Lưu vào database.
- [x] Hỗ trợ upload attachment qua API.
- [x] Thêm GET endpoints cho frontend lấy draft và parsed state.
- [x] Dời evidence table sang phase frontend/UI.
...
- [x] Case input, attachments, và parsed state được persist qua API backend mà không cần UI.
- [x] Attachment upload, PDF parsing cơ bản, và read endpoints đủ để lane runtime tích hợp không cần frontend thật.
...
- [x] Xây LangGraph flow.
- [x] Xây Plaintiff / Prosecutor agent.
- [x] Xây Defense agent.
- [x] Xây Judge agent.
- [x] Xây Clerk agent.
- [x] Xây Legal Retrieval agent.
- [x] Enforce turn order và role permissions cơ bản.

Deliverables:

- [x] Working simulation runtime
- [x] Structured agent outputs ở mức fixture/schema
- [x] Trial minutes draft ở mức fixture

Acceptance criteria:

- [x] Chạy được flow đầy đủ trên case mẫu.
- [x] Agent outputs không phải free-form hoàn toàn, mà bám schema.
- [x] Mỗi claim quan trọng có evidence hoặc citation đi kèm trong runtime output.
...
- [x] Xây Citation Verifier.
- [x] Xây Unsupported Claim Detector.
- [x] Xây Contradiction Checker.
- [x] Xây Outdated Legal Basis Warning.
- [x] Xây Audit Logger.
- [x] Xây Human Review Gate.
...
- [x] Citation không tồn tại trong retrieved set bị reject.
- [x] Claim không có evidence bị flag.
- [x] Citation hết hiệu lực bị warning.
- [x] Case rủi ro cao bị chặn ở human review gate.
...
- Backend MVP scope: complete and demoable.
- Frontend MVP scope: complete for MVP acceptance in `frontend/`.
- Plan-level MVP closure: closed after the MVP/V1/V2 mode UI was connected to backend flows.

Tasks:

- [x] Hoàn thiện UI panels.
...
- [x] UI hiển thị rõ evidence, citations, disputed points và review flags.
...
| Lane B | Backend Domain | Backend/platform engineer | Phase 2 completed | Ready now |
| Lane C | Agent Runtime | Agent/runtime engineer | Phase 3 usable | Ready now |
| Lane D | Safety and Evaluation | Safety/eval engineer | Phase 4 usable | Ready now |
...
- [x] `Milestone C`: Case ingestion usable
- [x] `Milestone D`: Structured simulation usable
- [x] `Milestone E`: Safety gate enforced
- [x] `Milestone F`: Demo-ready MVP
  Note: backend scripted demo and frontend MVP flow are both complete; UI supports case creation, upload, parse, simulate, review, and report preview.
