...
- [ ] Frontend workspace thật chưa khởi tạo; lane UI vẫn pending.
...
- Frontend MVP scope: pending.
- Plan-level MVP closure: still open until UI lane is built and `Milestone F` is checked.
...
- [ ] `Milestone F`: Demo-ready MVP
  Note: backend scripted demo is complete; milestone stays open only because the frontend/UI lane is still pending.
