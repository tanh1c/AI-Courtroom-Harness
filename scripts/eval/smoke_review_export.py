...
def main() -> None:
    ...
    temp_dir = Path(tempfile.mkdtemp(prefix="ai_court_phase5_"))
    ...
    review_response = client.post(
        ...
        json={
            ...
            "decision": "approve",
            "notes": "Baseline backend review approved for markdown export.",
            "checklist_updates": [
                "Reviewer confirmed the current baseline report can be exported for demo use."
            ],
        },
    )
    ...
    fetch_response = client.get(f"/api/v1/reports/{case_id}/markdown")
    fetch_response.raise_for_status()
    fetched = fetch_response.json()

    print("case_id:", case_id)
    print("pre_review_status:", simulation["case"]["status"])
    print("post_review_status:", review["report_status"])
    print("human_review_blocked:", review["human_review"]["blocked"])
    print("markdown_path:", exported["markdown_path"])
    print("markdown_saved:", Path(exported["markdown_path"]).exists())
    print("markdown_report_status:", fetched["report_status"])
    print("markdown_has_title:", fetched["markdown"].splitlines()[0])
    print("reviewer_name:", review["review_record"]["reviewer_name"])
